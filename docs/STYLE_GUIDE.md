...
1. Usuário acessa `/landing` → clica em "Entrar"
2. Formulário em `/login` → chama `authService.login()`
3. Mock valida apenas o `login` (sem senha real)
4. `AuthContext` salva o usuário em `sessionStorage`
5. Rotas protegidas conferem `isAuthenticated` via `ProtectedRoute`
6. Logout limpa `sessionStorage` e redireciona para `/landing`
